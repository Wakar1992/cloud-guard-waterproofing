...
function Testimonials() {
  return (
    ...
    >
      ...
      whileInView='visible'
      variants={slideUpVariants}
      className='lg:w-full w-[90%] grid lg:grid-cols-3 grid-cols-1 justify-center items-start
      gap-[20px] mt-[30px]'
      >
        {
          clients.map((item, index)=>(
            <div className='flex flex-col justify-center items-center' key={index}>
              <div className='border-2 border-white hover:bg-yellow-500 pb-[100px] pt-[30px]'>
                <p className='text-center text-white text-lg hover:text-black'>{item.about}</p>
              </div>
              <div className='flex flex-col justify-center items-center gap-[5px]'>
                ...
                <h1 className='text-yellow-500 text-[22px] font-semibold'>{item.post}</h1>
              </div>
            </div>
          ))
        }
